<div x-data="{}" x-init="@if (session('show_modal') === 'create-accountexecutive') $nextTick(() => $flux.modal('create-accountexecutive').show()) @endif">
    <flux:modal name="create-accountexecutive" class="md:w-96">

        <form action="{{ route('admin.users.store-account-executive') }}" method="POST" class="space-y-6" id="create-form">
            @csrf
            ...
            <div class="flex">
                <flux:spacer />
                <flux:button type="submit" id="create-button" variant="primary">
                    Create Account
                </flux:button>
            </div>
        </form>
