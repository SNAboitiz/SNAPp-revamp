<div x-data="{}" x-init="@if (session('show_modal') === 'edit-admin-modal') $nextTick(() => $flux.modal('edit-admin-modal').show()) @endif">
    <flux:modal name="edit-admin-modal" class="md:w-96">
        <form data-base-action="/admin/users/:user_id/update-admin" method="POST" id="edit-admin-form" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <flux:field>
                ...
                <flux:input name="edit_name" placeholder="Enter admin name" />
                @error('edit_name')
            ...
            <flux:field>
                ...
                <flux:select id="edit_customer_id" name="edit_customer_id" placeholder="— Select account —"
